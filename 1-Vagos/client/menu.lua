...
function Menuf6Vagos()
    local MainVagos = RageUI.CreateMenu("", "Interactions", X, Y, "Gang", "Gang", nil, nil, nil, nil)
    local InventaireVagos = RageUI.CreateSubMenu(MainVagos, " ", "Interactions")
    local MenotteVagos = RageUI.CreateSubMenu(MainVagos, " ", "Interactions")
    local VehiculeVagos = RageUI.CreateSubMenu(MainVagos, " ", "Interactions")
    RageUI.Visible(MainVagos, not RageUI.Visible(MainVagos))
    while MainVagos do
        Citizen.Wait(0)

            RageUI.IsVisible(MainVagos, true, true, true, function()
            ...
            RageUI.Separator("Action Fouille")
                ...
                RageUI.ButtonWithStyle("Personnalisé", "~b~Passe une annonce au nom de la Vagos !", {RightLabel = nil}, true, function(Hovered, Active, Selected)
                    if (Selected) then
                        local msg = KeyboardInput("Message", "", 100)
                        TriggerServerEvent('Vagos:Perso', msg)
                    end
                end)
                        ...
                        RageUI.IsVisible(InventaireVagos, true, true, true, function()
                    ...
                    RageUI.IsVisible(MenotteVagos, true, true, true, function()
            ...
            RageUI.ButtonWithStyle("→~s~ ~p~Menotter", nil, {RightLabel = nil}, closestPlayer ~= -1 and closestDistance <= 3.0, function(Hovered, Active, Selected)
                if Active then
                    ...
                    if (Selected) then
                            ...
                            playerCoords = GetEntityCoords(PlayerPedId())
                            if distance <= 2.0 then
                        TriggerServerEvent('Vagos:mettremenotte', target_id, playerheading, playerCoords, playerlocation)
                        else
                            Wait(500)
                            RageUI.CloseAll()
                            RageUI.Popup({message = "Personne autour de vous"})
            ...
            RageUI.ButtonWithStyle("→~s~ ~p~Démenotté", nil, {RightLabel = nil}, closestPlayer ~= -1 and closestDistance <= 3.0, function(Hovered, Active, Selected)
                if Active then
                    ...
                    if (Selected) then
                            ...
                            playerCoords = GetEntityCoords(PlayerPedId())
                            if distance <= 2.0 then  
                        TriggerServerEvent('Vagos:enlevermenotte', target_id, playerheading, playerCoords, playerlocation)
                        else
                            Wait(500)
                            RageUI.CloseAll()
                            RageUI.Popup({message = "Personne autour de vous"})
            ...
            RageUI.ButtonWithStyle("→~p~Escorter", "~b~Trainer la personne avec le marker (Uniquement avec menotte)", {RightLabel = nil}, closestPlayer ~= -1 and closestDistance <= 3.0, function(Hovered, Active, Selected)
                if Active then
                    ...
                    if (Selected) then
                            ...
                            playerCoords = GetEntityCoords(PlayerPedId())
                            if distance <= 2.0 then  
                    TriggerServerEvent('Vagos:drag', GetPlayerServerId(closestPlayer))
                        else
                            Wait(500)
                            RageUI.CloseAll()
                            RageUI.Popup({message = "Personne autour de vous"})
                    ...
                    RageUI.IsVisible(VehiculeVagos, true, true, true, function()
            ...
            RageUI.ButtonWithStyle("→~p~Mettre dans un véhicule", nil, {RightLabel = nil}, closestPlayer ~= -1 and closestDistance <= 3.0, function(Hovered, Active, Selected)
                if Active then
                    ...
                    if (Selected) then
                            ...
                            playerCoords = GetEntityCoords(PlayerPedId())
                            if distance <= 2.0 then  
                    TriggerServerEvent('Vagos:putInVehicle', GetPlayerServerId(closestPlayer))
                        else
                            Wait(500)
                            RageUI.CloseAll()
                            RageUI.Popup({message = "Personne autour de vous"})
            ...
            RageUI.ButtonWithStyle("→~p~Sortir du véhicule", nil, {RightLabel = nil}, closestPlayer ~= -1 and closestDistance <= 3.0, function(Hovered, Active, Selected)
                if Active then
                    ...
                    if (Selected) then
                            ...
                            playerCoords = GetEntityCoords(PlayerPedId())
                            if distance <= 2.0 then  
                    TriggerServerEvent('Vagos:OutVehicle', GetPlayerServerId(closestPlayer))
                        else
                            Wait(500)
                            RageUI.CloseAll()
                            RageUI.Popup({message = "Personne autour de vous"})
                ...
                if not RageUI.Visible(MainVagos) and not RageUI.Visible(InventaireVagos) and not RageUI.Visible(MenotteVagos) and not RageUI.Visible(VehiculeVagos) then
                    MainVagos = RMenu:DeleteType("Vagos", true)
        end
    end
end

Keys.Register('F6', 'Vagos', 'Ouvrir le menu Vagos', function()
...
function carteidentite(player)
    local CarteVagos = RageUI.CreateMenu("Carte d'identité", "Informations")
    ESX.TriggerServerCallback('Vagos:getOtherPlayerData', function(data)
    RageUI.Visible(CarteVagos, not RageUI.Visible(CarteVagos))
        while CarteVagos do
            Citizen.Wait(0)
                RageUI.IsVisible(CarteVagos, true, true, true, function()
            ...
            if not RageUI.Visible(CarteVagos) then
            CarteVagos = RMenu:DeleteType("Carte d'identite", true)
        end
    end
